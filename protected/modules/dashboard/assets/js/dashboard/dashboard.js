	...
	load : function() {
		...
		$("#add-card").click(function(e) {
			...
			$.post(CiiDashboard.endPoint + "/default/getCardsByCategory/id/" + $(".menu li:first-child").find("a").attr("href").replace("#", ""), function(data) {
				...
				$("select").imagepicker();
				CiiDashboard.bindAddCardsButton();
			});
			return false;
		});
		...
		$.get(CiiDashboard.endPoint + "/card/getCards", function(data) {
			...
			CiiDashboard.enableDragBehavior();
			CiiDashboard.bindDeleteBehavior();
			CiiDashboard.bindSettingsBehavior();
			CiiDashboard.bindFlipEvent();
	 	});
	},
 		...
 		$("#add-cards-button").click(function(e) {
 			...
 			$(items).each(function(key, value) {
 				$.post(CiiDashboard.endPoint + "/card/add/id/" + value, function(data) {
 					...
 					CiiDashboard.enableDragBehavior();
 					CiiDashboard.bindResizeBehavior();
					CiiDashboard.bindDeleteBehavior();
					CiiDashboard.bindSettingsBehavior();
					CiiDashboard.bindFlipEvent();
					CiiDashboard.rebuild(true);
 					$("#add-card").click();
 				});
 			});
 		});
