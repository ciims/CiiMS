...
class CiiActiveForm extends TbActiveForm
{
	...
	public function toggleButtonRow($model, $property, $htmlOptions=array(), $validators=NULL)
	{
		...
		echo CHtml::openTag('div', array('class' => 'pure-input-2-3', 'style' => 'display: inline-block'));
			echo CHtml::openTag('label', array('class' => 'switch-light checkbox toggle candy blue', 'onclick' => ''));
				echo CHtml::tag('input', CMap::mergeArray($htmlOptions, array('type'=>'checkbox', 'id' => get_class($model) . '_' . $property, 'name' => get_class($model) . '[' . $property . ']', 'value' => $model->$property)), NULL);
